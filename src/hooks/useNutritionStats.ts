...
export function useNutritionStats(daysBack: number = 7) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<NutritionStats> => {
      ...
      // Group by date
      const byDate = new Map<string, { totalCalories: number; totalProtein: number; mealCount: number }>();
      for (const entry of data || []) {
        const existing = byDate.get(entry.entry_date) || { totalCalories: 0, totalProtein: 0, mealCount: 0 };
